...
def user_login(request):
  top = Profile.objects.all().order_by("-points")[:min(Profile.objects.count(),5)] #gets the top 5 rated users
  if request.user.is_authenticated:
    return HttpResponseRedirect('/')
  if request.method == 'POST': #registration
    if 'email' in request.POST:  #attributes of register page
      ...
      if password==conf_password: #password and confirm-password must match
        if User.objects.filter(email=email).exists(): # no user with same mail id
          ...
          return render(request,'login.html',{}) # render the login page after registration
        if User.objects.filter(username=username).exists():
          messages.error(request, "Username already in use.")
          return render(request,'login.html',{})
        user = User.objects.create_user(username, email, password) #creating a user in database
        user.save() #saving
        profile = Profile(user=user) #user is a foreign key to Profile
        profile.save() #save the profile
        messages.success(request, "Account created.") 
      else:
        messages.error(request, "Passwords do not match.")
      # return the login page with total questions and answers 
      return render(request,'login.html',
...
def Question_create(request):
  ...
  if request.method == 'POST':
    ...
    ques = Question(user=instance,question=question, description=des) #create question ORM
    ques.save()
